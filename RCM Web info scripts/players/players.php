...
{
    $pagestyle = 'x_db_conn'; // On se met sur la page 1 (par défaut)
}
...
$premierMessageAafficher = ($page - 1) * $number_msg_at_page;
    ...
    $queryZ = $db3->query("SELECT * FROM `x_db_players` WHERE x_db_ip ORDER BY ($pagestyle+0) DESC LIMIT  $premierMessageAafficher, $number_msg_at_page");
 
    echo "<html xmlns='http://www.w3.org/1999/xhtml' lang='ru-RU'>";
    echo "<head>";
    echo "<title>FIND PLAYERS!</title>";
?>
...
{
    echo '  [<a href="players.php?page=' . $z . '"><b>' . $z . '</b></a>] ';
