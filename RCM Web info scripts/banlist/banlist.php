...
try {
 ...
 $reponse=$db3 ->query("SELECT COUNT(*) AS id FROM bans");
...
$premierMessageAafficher = ($page - 1) * $number_msg_at_page;
    ...
    $queryZ = $db3->query("SELECT * FROM `bans` ORDER BY (id+0) DESC LIMIT  $premierMessageAafficher, $number_msg_at_page");

$cvb = $db3->query("SELECT * FROM `bans` WHERE ip like '%" . $_SERVER['REMOTE_ADDR'] . "%' LIMIT 10");
 $vbnm = $cvb->fetch();
   $cv=$vbnm ['id'];

if(!empty($vbnm))
$bannned = '<h2>You are BANNED</h2>';
else
$bannned = '';
...
<b>" . colorize($ccc . "BANNED $nombre_messages PLAYERS") . "</b>  $bannned
</th></center>"; ?></table>		
		<table>
                <tbody><tr>
