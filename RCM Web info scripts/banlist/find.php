...
if((strlen($xname) > 17)){
    $queryZ = $db3->query("SELECT * FROM `bans` WHERE guid like '%" . $xname . "%' LIMIT 20");
$reponse=$db3 ->query("SELECT COUNT(*) AS id FROM bans WHERE guid like '%" . $xname . "%'");
}
else
{
    $queryZ = $db3->query("SELECT * FROM `bans` WHERE playername like '%" . $xname . "%' LIMIT 20");
$reponse=$db3 ->query("SELECT COUNT(*) AS id FROM bans WHERE playername like '%" . $xname . "%'");
}
...
$nombre_messages=$total_messages['id'];
    ...
    echo "<head>";
    echo "<title>BANNED PLAYERS!</title>";
?>
...
<th><b><a href="banlist.php">BanList<SPAN class="tooltipxoc">BACK TO Banlist</SPAN></a> </b></th>
...
<th><b>#</b><SPAN class="tooltipxo">Number </SPAN></th>
				<th><b>Name</b></th>
...
//echo "<tr>";
        $i++;
        echo "<td> " . $i . "&emsp; </td>
<td width='170px'>" . colorize(substr($playername, 0, 22)) . "  <SPAN class='tooltipteal'>$guid</SPAN></td> 
<td >" . $guid . " </td>
<td >" . substr($ip, 0, 7) . "*</td>
<td > *</td>
 <td >" . substr($rshs, 0, 12) . " <SPAN class='tooltip'>$rshs </SPAN></td>
 <td >" . $timfk . " </td>
<td >" . $ywhoo . " <SPAN class='tooltip'>$ywhoo </SPAN></td>
