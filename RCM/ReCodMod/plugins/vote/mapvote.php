...
$x_namex = trim(clearnamex($i_name));
	  usleep($sleep_rcon);
	  rcon('say ^6 ^7'.$nickr.' ^3VOTED, It is useless to make repeat!', '');
	}	
...
if(($razn < $v_time_map) && (strpos($msgr,$ixz.'xmap ') !== false)) 
  ...
  usleep($sleep_rcon);
  rcon('say ^6 '. $votm . ' "^2Vote reset. Dont use '.$ixz.'xmap command in vote time!', '');
...
++$x_stop_lp;    //return;
 ...
 if (($msgr == $cmdyes) || ($msgr == "++") || ($msgr == "+++")){
  rcon('say ^6 ^2Vote time is over, get starting vote with ^7'.$ixz.'xmap ^2command!', '');
 }else{
...
fclose($fh);  
 ...
 $db1->exec("UPDATE x_cmd_kck SET z_counts='$xcnnn',z_time='$xtee'");
  usleep($sleep_rcon);
  rcon('say ^6 '. $votm . ' "^2Vote Activated!: ^7'.$ixz.'xmap '.$idgz.'"^1<- Vote with ^7"'.$cmdyes.'"^2 or ^7'.$cmdnoo.' ^2for change to ^3'.$idgz.'', '');
